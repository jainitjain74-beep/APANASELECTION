...
(function () {
  ...
  /* Runs a list of init/render functions one at a time. If one throws
     (e.g. because of a mismatched or partially-updated HTML file), the
     rest still run instead of the whole page silently breaking after
     the failure point. */
  function runSteps(steps) {
    steps.forEach((step) => {
      try {
        ...
        console.error("Step failed:", step.name || step, err);
      }
    });
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (page === "site") {
      ...
      runSteps([initTheme, initLoginPage]);
    } else if (page === "dashboard") {
      runSteps([initTheme, initDashboardPage]);
    }
  });
})();
